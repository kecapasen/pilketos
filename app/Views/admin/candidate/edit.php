...
<div class="w-full p-4 lg:p-8">
    ...
    <?= view('components/page_header', [
        'title'    => 'Edit Kandidat',
        'subtitle' => 'Ubah data pasangan calon nomor ' . $candidate->order_number,
    ]) ?>
    ...
    <div class="bg-surface rounded-2xl border border-primary overflow-hidden">
        <div class="p-4 border-b border-primary bg-input flex justify-between items-center">
            <h3 class="font-bold text-primary text-sm uppercase tracking-wider">
                <i class="bi bi-pencil-fill me-2"></i>Edit Data Kandidat
            </h3>
        ...
        <form action="<?= base_url('admin/candidate/update/' . $candidate->id) ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="number" name="order_number" value="<?= old('order_number', $candidate->order_number) ?>" required min="1"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="chairman_name" value="<?= old('chairman_name', $candidate->chairman_name) ?>" required
                    ...
                    <div>
                        ...
                        <input type="text" name="chairman_class" value="<?= old('chairman_class', $candidate->chairman_class) ?>" required
                               class="w-full bg-input border border-primary text-primary text-sm rounded-xl py-3 px-4 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="vice_chairman_name" value="<?= old('vice_chairman_name', $candidate->vice_chairman_name) ?>" required
                               class="w-full bg-input border border-primary text-primary text-sm rounded-xl py-3 px-4 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all"
                    ...
                    <div>
                        ...
                        <input type="text" name="vice_chairman_class" value="<?= old('vice_chairman_class', $candidate->vice_chairman_class) ?>" required
                               class="w-full bg-input border border-primary text-primary text-sm rounded-xl py-3 px-4 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all"
                ...
                <div>
                    ...
                    <textarea name="vision" required rows="3"
                              ...
                              placeholder="Tuliskan visi pasangan calon..."><?= old('vision', $candidate->vision) ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="mission" required rows="5"
                              class="w-full bg-input border border-primary text-primary text-sm rounded-xl py-3 px-4 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all resize-none"
                              placeholder="Tuliskan misi pasangan calon (pisahkan dengan enter untuk poin-poin)..."><?= old('mission', $candidate->mission) ?></textarea>
                </div>

                <!-- Foto -->
                <div>
                    <label class="block text-xs font-bold text-primary uppercase tracking-wider mb-2">Foto Paslon</label>
                    <div class="flex items-start gap-4">
                        <div id="imagePreview" class="w-24 h-24 bg-primary/10 border border-primary rounded-xl flex items-center justify-center overflow-hidden shrink-0">
                            <img src="<?= base_url('uploads/candidates/' . $candidate->image) ?>" class="w-full h-full object-cover">
                        </div>
                        <div class="flex-1">
                            <div class="border-2 border-dashed border-primary rounded-xl p-4 text-center hover:border-accent hover:bg-accent/5 transition-all cursor-pointer" id="dropZone">
                                ...
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    <p class="text-[10px] text-primary">Biarkan kosong jika tidak ingin mengubah. Format: JPG, JPEG, PNG. Max 2MB</p>
                                </div>
                            </div>
